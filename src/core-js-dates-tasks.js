...
function getNextFriday(date) {
  ...
  let delta;
  if (currWeekDay < friday) {
    delta = friday - currWeekDay;
  } else {
...
function getCountDaysInMonth(month, year) {
  ...
  if (month === 2) {
    date.setDate(29);
    if (date.getMonth() === monthIndex) return 29;
    return 28;
  }
  date.setDate(31);
  if (date.getMonth() === monthIndex) return 31;
  return 30;
}
...
function getCountWeekendsInMonth(month, year) {
  const monthIndex = month - 1;
  let currentDate = 1;
  let date = new Date(year, monthIndex, currentDate);
  let weekendDays = 0;
...
function getWeekNumberByDate(date) {
  ...
  startOfYear.setDate(1 + ((4 - startOfYear.getDay() + 7) % 7));
  const weekNum =
    1 +
    Math.round(
      (date.getTime() - startOfYear.getTime()) / (1000 * 60 * 60 * 24 * 7)
    );
  return weekNum;
}
...
function getNextFridayThe13th(date) {
  ...
  currDate.setDate(currDate.getDate() + ((5 - currWeekDay + 7) % 7));
  currDate.setDate(currDate.getDate() + 7);
  while (currDate.getDate() !== 13) {
    currDate.setDate(currDate.getDate() + 7);
  }
  return currDate;
}
  ...
  while (date.getTime() <= endDate.getTime()) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (date.getTime() <= endDate.getTime()) {
        schedule.push(date.toLocaleDateString('en-GB').split('/').join('-'));
      }
